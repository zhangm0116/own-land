...
{
	rt_device_write(uart2_dev, 0, &c, 1);
	return 0;
}
...
int uart_ctl_init(void)
{
	...
	{
		rt_device_open(uart2_dev, RT_DEVICE_OFLAG_RDWR);
	}
	else
	{
	...
	rt_device_set_rx_indicate(uart2_dev, uart_input);
...
static void len_error(void)
{
	...
	rt_device_write(uart2_dev, 0, tdata, 5);
...
void uart_ctl_thread_task(void* parameter)
{
	...
	for (;;)
	{
		if (rt_sem_take(&sem_rx, 10) == RT_EOK)
		{
			while (rt_device_read(uart2_dev, 0, &ch, 1) == 1)
			{
				...
				else
				{
					...
					if (rd_index > rd_data[2] + 4)
					{
						...
						if (checkdata != rd_data[offset])
						{
							...
							rt_device_write(uart2_dev, 0, tx_data, 5);
                ...
                if_get_dev_status(&rd_bb_status, sizeof(struct dev_status_t));
		            ...
		            memcpy(&tx_data[4], &rd_bb_status, sizeof(struct dev_status_t));
							...
							case GET_COMMAND_STATUS:
								rt_kprintf("read cmd :0x%x\n",cmd_rd);		
								tx_data[0] = 0x55;
								tx_data[1] = 0xaa;
								tx_data[2] = 1;
								tx_data[3] = 0x80;
								tx_data[4] = cmd_rd & 0xff;
	 
								tx_data[5] = check_sum(&tx_data[0],5);
								rt_device_write(uart2_dev,0,tx_data,6);
								break;
							case MCU_FLASH_ERASE:
								rt_kprintf("Start erase mcu flash...\n");
							
								MCU1_upgrade_enable(1);
								MCU1_reset();
