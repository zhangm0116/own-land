...
static void tcpip_init_done_callback(void *arg)
{
    ...
    information = &rt_object_container[RT_Object_Class_Device]; //将统一管理的网口名称赋值给information list
    for (node = information->object_list.next;
         node != &(information->object_list);
         node = node->next)                                    //遍历整个列表  将网口添加到网卡列表
    {
        ...
        if (device->type == RT_Device_Class_NetIf)
        {
            ...
            netif_add(ethif->netif, &ipaddr, &netmask, &gw,
                      ethif, netif_device_init, tcpip_input); //向网口列表中添加一个网口  mac地址初始化 

            if (netif_default == RT_NULL)
                netif_set_default(ethif->netif);  //设置默认网口
            ...
            {
                /* set interface up */
                netif_set_up(ethif->netif);  //使能网口通讯处理
            }
    ...
    rt_sem_release((rt_sem_t)arg);  //释放信号量  
...
int lwip_system_init(void)
{
   ...
   rt_err_t rc;
    ...
    tcpip_init(tcpip_init_done_callback, (void *)&done_sem);  //协议栈初始化  tcpip_init_done_callback  初始化完成后的回调函数

	
		
    /* waiting for initialization done */
    if (rt_sem_take(&done_sem, RT_WAITING_FOREVER) != RT_EOK)  //说明初始化没有完成
    {
    ...
    rt_sem_detach(&done_sem);            //脱离信号量

    /* set default ip address */
#if !LWIP_DHCP   //如果没有使用自动获取IP 
    if(netif_default != RT_NULL)
    {
        struct ip_addr ipaddr, netmask, gw;  

        IP4_ADDR(&ipaddr, RT_LWIP_IPADDR0, RT_LWIP_IPADDR1, RT_LWIP_IPADDR2, RT_LWIP_IPADDR3);  //ip地址
        IP4_ADDR(&gw, RT_LWIP_GWADDR0, RT_LWIP_GWADDR1, RT_LWIP_GWADDR2, RT_LWIP_GWADDR3);      // 默认网关
        IP4_ADDR(&netmask, RT_LWIP_MSKADDR0, RT_LWIP_MSKADDR1, RT_LWIP_MSKADDR2, RT_LWIP_MSKADDR3); // 子网掩码

        netifapi_netif_set_addr(netif_default, &ipaddr, &netmask, &gw);  //给默认网口添加地址
