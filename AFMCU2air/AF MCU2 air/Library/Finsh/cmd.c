...
int get_hex ( char * buf, unsigned int *address) 
{
    int done = 0; 
    unsigned int cpos = 0; 
...
void cmd_phy_bd(int argc, char** argv)
{
	rt_uint16_t bsr, bcr;
	rt_uint16_t phy_id_low,phy_id_high;
	rt_uint32_t phy_id;
	unsigned int addr;
	...
	{
			get_hex(argv[1], &addr);
			rt_kprintf("Read PHY addr 0x%x\n",addr);
		
			bsr = ETH_ReadPHYRegister(addr, PHY_BSR);
			bcr = ETH_ReadPHYRegister(addr, PHY_BCR);
			
			phy_id_low = ETH_ReadPHYRegister(addr, PHY_LOW);
			phy_id_high = ETH_ReadPHYRegister(addr, PHY_HIGH);
			
			phy_id = (phy_id_high << 16) | phy_id_low;
			
			rt_kprintf("PHY ID : 0x%x\n",phy_id);
			
			rt_kprintf("BCR = 0x%X\tBSR = 0x%X\tPHY_CR = 0x%X\n", bcr,bsr);
	}
	

//	rt_kprintf("PHY_FCSCR = 0x%X\n", ETH_ReadPHYRegister(0x05, PHY_FCSCR	) );
//	rt_kprintf("PHY_MISR = 0x%X\n", ETH_ReadPHYRegister(0x05, PHY_MISR ) );

}
FINSH_FUNCTION_EXPORT_ALIAS(cmd_phy_bd, __cmd_phy_bd, Show PHY register.);

void cmd_fc_100m(int argc, char** argv)
{
	rt_uint16_t bsr, bcr;
	unsigned int addr;
	...
	if(argc == 2)
	{
		...
		ETH_WritePHYRegister(addr, PHY_BCR,0x2100);
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_fc_100m, __cmd_fc_100m, Foce PHY 100M full duplex.);
...
int cmd_rd_phy(int argc, char** argv)
{
		unsigned int value;
    ...
    if(argc == 3)
    {
        get_hex(argv[1], &phy_addr);
				get_hex(argv[2], &reg_addr);
				rt_kprintf("PHY addr:0x%x\n",phy_addr);
				rt_kprintf("reg addr:0x%x\n",reg_addr);
        //reg = if_read((uint8_t)addr);
				value = ETH_ReadPHYRegister(phy_addr, reg_addr);
        rt_kprintf("phy value 0x%x\n",value);
    }
    return 0;
}
FINSH_FUNCTION_EXPORT_ALIAS(cmd_rd_phy, __cmd_rd_phy, read phy reg value.);

int cmd_wr_phy(int argc, char** argv)
{
    unsigned int value;
    unsigned int phy_addr;
		unsigned int reg_addr;
    ...
    if(argc == 4)
    {
        get_hex(argv[1], &phy_addr);
				get_hex(argv[2], &reg_addr);
        get_hex(argv[3], &value);
        //if_write((uint8_t)addr, value);
				rt_kprintf("PHY addr:0x%x\n",phy_addr);
				rt_kprintf("reg addr:0x%x\n",reg_addr);
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_wr_phy, __cmd_wr_phy, write phy reg value.);
