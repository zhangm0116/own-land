...
static struct rt_stm32_eth stm32_eth_device;   //test   static   
...
static void ETH_MACDMA_Config(void)
{
  ...
  ETH_InitStructure.ETH_AutoNegotiation = ETH_AutoNegotiation_Disable;  //自适应关闭
...
//   ETH_InitStructure.ETH_ChecksumOffload = ETH_ChecksumOffload_Enable;                //IPV4???????
//   #endif
	...
	//dma  
	ETH_InitStructure.ETH_DropTCPIPChecksumErrorFrame = ETH_DropTCPIPChecksumErrorFrame_Enable;  //test
...
void rt_hw_stm32_eth_init(void)
{
  ...
  sn0=*(vu32 *)(0x1FFF7A10);
	...
	stm32_eth_device.dev_addr[3]=(sn0>>16)&0XFF;   //低三字节用STM32的唯一ID
...
////	stm32_eth_device.dev_addr[5] = 0x89;     //  天空   0x89    地面0x88 
	...
	/* register eth device */   
	 //通过该函数会调用 eth_device_init_with_flag  函数初始化网口netif 赋值输出函数
...
void ETH_IRQHandler(void)
{
	...
	if (ETH_GetDMAFlagStatus(ETH_DMA_FLAG_R) == SET)
	{
		...
		result = eth_device_ready(&(stm32_eth_device.parent));      // 改变接收邮箱状态
	...
	if (ETH_GetDMAITStatus(ETH_DMA_IT_T) == SET) /* packet transmission */  // 发送中断
	{
